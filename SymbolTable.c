...
int checkedInsertionLocalVar(unsigned int scope,const char* name, unsigned int line,enum SymbolType type,int isFunction){
    if(checkIfLibFunction(name)==0){return -2;}//  LIBFUNC OVERIDE ERROR
    if(checkedInsertionVariable(scope,name,line,type,isFunction)==-1)return -1;
    else return 0;
}
...
int checkedInsertionFormal(unsigned int scope,const char* name, unsigned int line,enum SymbolType type){
...
int checkedInsertionFunction(unsigned int scope,const char* name, unsigned int line,enum SymbolType type){
    if(checkIfLibFunction(name)==0){
        printf("ERROR:::::: Variable with name: %s is a libfunc!!!!!\n",name);
        return -1;
    }//  LIBFUNC OVERIDE ERROR
...
int checkedInsertionVariable(unsigned int scope,const char* name, unsigned int line,enum SymbolType type,int isFunction){
    ...
    for(i=scope;i>0;i--){
        ...
        if(i==scope&&type==5&&tmp==NULL){
            ...
            return 0; // uparxei idi i local metavliti sto scope, opote dn tin ksanavazw (alla den einai kai lathos)
        }
        if(tmp!=NULL){
...
int checkedInsertion(unsigned int scope,const char* name, unsigned int line,enum SymbolType type,int isFunction){
    ...
    if(type==0){
        ...
        if(checkedInsertionFormal(scope,name,line,type)==0)return 0;
        else return -1;
    }else if(type==3){
        if(checkedInsertionFunction(scope,name,line,type)==0)return 0;
        else return -1;
    }else if(type==4){
    }
}
